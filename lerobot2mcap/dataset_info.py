...
class DatasetInfo:
    ...
    def __init__(self, info_json_path: Path):
        ...
        self.dataset_root = (
            info_json_path.parent.parent
        )  # meta/info.json -> dataset_root
        self.data = (
    ...
    def get_episode_info(self, episode_index: int) -> dict:
        ...
        if self._episodes_df is None:
            raise ValueError(
                "Episodes metadata not loaded. "
                "This method is only available for v3.0 datasets."
            )

        episode_row = self._episodes_df[
            self._episodes_df["episode_index"] == episode_index
        ]
    ...
    def get_episode_files(
        ...
        if version.startswith("v2"):
            ...
            if self._episodes_df is None:
                raise ValueError(
                    "Episodes metadata not loaded. Cannot get episode files for v3.0 dataset."
                )
