...
class LeRobotConverter:
    ...
    def _convert_episode_v3(self, episode_idx: int, output_dir: Path) -> Path:
        ...
        with tempfile.TemporaryDirectory(
            prefix=f"lerobot2mcap_ep{episode_idx}_"
        ) as temp_dir:
            ...
            episode_config = self.config_generator.generate_episode_config_v3(
    ...
    def _extract_episode_parquet(
        ...
        if episode_df.empty:
            raise ValueError(
                f"No data found for episode {episode_idx} "
    ...
    def _extract_episode_parquet_split(
        ...
        state_cols = ["observation.state"] + [
            c for c in metadata_cols if c in episode_df.columns
        ]
